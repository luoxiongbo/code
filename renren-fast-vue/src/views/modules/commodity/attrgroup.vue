  ...
  <el-row :gutter="20">
    <el-col :span="5">
      <!--1. 分类树形占 5 个单元宽度, 保留 ref, :data, :props, node-key 基本显示的属性
          2. node-click 实际是 el-tree 中规定好的实际，会传递三个参数: 参考 elmentui 官网 elementui-Tree 树形控件-Events -->
      ...
      <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
          ...
          <el-form-item>
            ...
            <el-button v-if="isAuth('commodity:attrgroup:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
            <el-button v-if="isAuth('commodity:attrgroup:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
          </el-form-item>
        </el-form>
        <el-table
          :data="dataList"
        ...
        <!-- 弹窗, 新增 / 修改 （⚠⚠⚠⚠⚠⚠：也就是attrgroup-add-or-update.vue 文件）-->
  ...
  export default {
    data () {
      return {
        ...
        catId: 0, // catId 默认=0, 表示后台查询分类属性组信息时，不加入 catId 条件
        dataList: [],
        pageIndex: 1,
        pageSize: 2,
        totalPage: 0,
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          ...
          // url: `http://localhost:9090/commodity/attrgroup/list/${this.catId}`,
          method: 'get',
          params: this.$http.adornParams({
            'page': this.pageIndex,
            'limit': this.pageSize,
            'key': this.dataForm.key // 这个 key 就是搜索框的值，可能为id也可能为name
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
